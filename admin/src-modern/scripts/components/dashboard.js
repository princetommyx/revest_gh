...
export class DashboardManager {
  ...
  async init() {
    console.log('🚀 Revesta Dashboard Manager initialized');
  ...
  async loadDashboardData() {
    try {
      // Import API client
      const { adminApi } = await import('../utils/api-config.js');
      ...
      console.log('✅ Dashboard data loaded from backend', stats);
    } catch (error) {
  ...
  updateStatsCards(stats) {
    // Update Total Users
    const totalUsersEl = document.querySelector('[data-stat-value]');
    if (totalUsersEl && stats.total_users !== undefined) {
      totalUsersEl.textContent = stats.total_users.toLocaleString();
    }

    // Update other stats in the dashboard
    // You can add more specific selectors here based on your HTML structure
