...
from .serializers import (
    ...
    UserLocationSerializer, ChangePasswordSerializer,
        ...
        if user:
            ...
            if user.email:
                try:
                    ...
                    threading.Thread(
                        target=_async_mail, args=(
                            user.email, otp_code), daemon=True).start()
...
class VerifyLoginOTPView(views.APIView):
    ...
    def post(self, request):
        ...
        if not user_id or not otp_code:
            return Response({"detail": "user_id and otp are required"},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if not verification.is_valid():
                return Response(
                    {"detail": "OTP has expired or already used"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({"detail": "Invalid verification session"},
                            status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            return Response({"detail": str(e)},
    ...
    def get(self, request):
        status_data = {
            ...
            'port': getattr(
                ...
                'EMAIL_PORT',
                'N/A'),
            'cors_origins': settings.CORS_ALLOWED_ORIGINS if hasattr(
                settings,
                'CORS_ALLOWED_ORIGINS') else 'Not Set'}
...
class GoogleLoginView(views.APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not id_info:
                return Response(
                    {'error': 'Invalid or expired Google token'}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if not email:
                return Response(
                    {'error': 'Email not found in Google response'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        ...
        try:
            ...
            if not user.expo_push_token:
                return Response(
                    {'error': 'User has no push token registered'}, status=status.HTTP_400_BAD_REQUEST)
...
class TestSMSView(views.APIView):
    ...
    def post(self, request):
        ...
        content = request.data.get(
            'content', 'This is a test message from Revesta Hubtel Integration.')
