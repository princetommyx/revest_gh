...
def send_push_notification(user, title, body, data=None, urgency='NORMAL', background=True):
    """
    Sends a push notification to the user and saves it in the database.

    `background=True` (the default) fires the Expo call on a daemon thread,
    which is right for request/response call sites - the process stays
    alive to let it finish. Short-lived scripts (management commands) must
    pass `background=False` so the send happens inline before the process
    exits, otherwise the interpreter can shut down mid-thread and the push
    never actually reaches Expo.
    """
    # 1. Save to Database
    ...
    if token:
        if background:
            ...
            _send_task(user.id, token, title, body, data, urgency)
