...
with open('mobile/src/screens/ListingDetailScreen.js', 'r') as f:
    content = f.read()
...
content = re.sub(r'import React.*?from \'../api/client\';', new_imports, content, flags=re.DOTALL)
...
content = re.sub(r'export default function ListingDetailScreen.*?const styles = StyleSheet\.create\({.*?\);', component + '\n\n' + styles, content, flags=re.DOTALL)

with open('mobile/src/screens/ListingDetailScreen.js', 'w') as f:
    f.write(content)
